...
class TestOtelInstrumentationFlag:
    def make_dbt_context(
        self, context_name: str, args: List[str], parent: Optional[click.Context] = None
    ) -> click.Context:
        return cli.make_context(context_name, args.copy(), parent)
    ...
    def test_default_off(self):
        ctx = self.make_dbt_context("run", ["run"])
        flags = Flags(ctx)
        assert flags.OTEL_INSTRUMENTATION is False

    def test_cli_arg_enables(self):
        ctx = self.make_dbt_context("run", ["--otel-instrumentation", "run"])
        flags = Flags(ctx)
        assert flags.OTEL_INSTRUMENTATION is True

    def test_env_var_enables(self, monkeypatch):
        monkeypatch.setenv("DBT_ENGINE_OTEL_INSTRUMENTATION", "True")
        ctx = self.make_dbt_context("run", ["run"])
        flags = Flags(ctx)
        assert flags.OTEL_INSTRUMENTATION is True

    def test_project_flags_enables(self):
        project_flags = ProjectFlags(otel_instrumentation=True)
        ctx = self.make_dbt_context("run", ["run"])
        flags = Flags(ctx, project_flags)
        assert flags.OTEL_INSTRUMENTATION is True
    ...
    def test_cli_overrides_project_flags(self):
        project_flags = ProjectFlags(otel_instrumentation=True)
        ctx = self.make_dbt_context("run", ["--no-otel-instrumentation", "run"])
        flags = Flags(ctx, project_flags)
        assert flags.OTEL_INSTRUMENTATION is False
